(() => {
  ...
  // ===== スマホ用仮想コントローラー（キーボード操作はそのまま維持） =====
  // タッチ対応端末を検出して <html> にクラスを付与（メディアクエリの保険）。
  // これで機種やブラウザによってメディアクエリが効かない場合でもボタンを表示できる。
  const isTouchDevice =
  ...
  // 左/上/右ボタンは押している間だけ対応する keys を true にする。
  // 既存の update() が keys を読むため、キーボードと同じ挙動になる。
  ...
  if (btnFire) {
    ...
    btnFire.addEventListener('pointerup', fireRelease);
    btnFire.addEventListener('pointercancel', fireRelease);
    btnFire.addEventListener('pointerleave', fireRelease);
